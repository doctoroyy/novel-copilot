/**
 * 文风启发式校验桥接 — 零 AI 调用的质量检测
 *
 * 来源: src/qc/writingStyleHeuristics.ts
 * 覆盖方法论中最容易机械检出的"弃书红线"。
 */
 ...
 * 分析章节文风质量 — 零 AI 调用。
 *
 * 返回 blocking/review 级别的问题和 8 项量化指标。
 */
export function analyzeWritingStyle(
  ...
  // Blocking reasons (弃书红线):
  if (maxSettingRunChars > 500) {
    blockingReasons.push(`存在 ${maxSettingRunChars} 字的无对白长段（设定/世界观倾泻，红线 300 字）`);
  ...
  if (dialoguePercent < 0.15 && bodyChars > 1500) {
    reviewReasons.push(`对话占比仅 ${(dialoguePercent * 100).toFixed(0)}%（<15%），章节过于叙述化`);
  }

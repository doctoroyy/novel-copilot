/**
 * 写作规则桥接 — 从核心引擎提取的纯逻辑函数
 *
 * 来源: src/writingRules.ts
 * 无 AI 调用，无外部依赖。
 */
...
【业内质量门槛（写完前自检，不输出）】
- 人物可信：角色知道什么、能做到什么、为什么这样做，必须与上下文一致
- 场景因果：每个场景都有目标、阻碍、行动、结果；禁止只堆设定/情绪/氛围
- 选择与代价：至少出现一次让角色付出代价或承担风险的选择
- 类型契约：本章必须兑现题材卖点，不得只写通用日常或空泛情绪
- 角色声音：主要人物说话要能区分，避免所有人同一种书面腔

...
type BibleGenerationRequestInput = {
  genreInput?: string;
  themeInput?: string;
  keywordsInput?: string;
  templateId?: string;
  templateSnapshotDate?: string;
  bodyTemplate?: ImagineTemplate | null;
};

type BibleGenerationTaskResult = {
  bible: string;
  templateApplied: {
    templateId: string;
    templateName: string;
...
type BibleGenerationJobRequestPayload = {
  request: BibleGenerationRequestInput;
  aiConfig: AIConfig;
};

type BibleGenerationJob = {
  id: string;
  userId: string;
  status: BibleGenerationJobStatus;
  message: string | null;
  errorMessage: string | null;
  resultBible: string | null;
  templateApplied: BibleGenerationTaskResult['templateApplied'];
  fallbackModelUsed: BibleGenerationTaskResult['fallbackModelUsed'];
  createdAt: number;
...
const BIBLE_JOB_WAITING_MESSAGE = '任务已创建，等待队列执行...';
const BIBLE_JOB_RUNNING_MESSAGE = '正在生成设定...';
...
function normalizeBibleGenerationJob(row: any): BibleGenerationJob {
  return {
    id: String(row.id || ''),
    userId: String(row.user_id || ''),
    status: (String(row.status || 'queued') as BibleGenerationJobStatus),
    message: row.message ? String(row.message) : null,
    errorMessage: row.error_message ? String(row.error_message) : null,
    resultBible: row.result_bible ? String(row.result_bible) : null,
    templateApplied: parseJsonSafely<BibleGenerationTaskResult['templateApplied']>(row.template_applied_json, null),
    fallbackModelUsed: row.fallback_model_provider || row.fallback_model_name
      ? {
        provider: String(row.fallback_model_provider || ''),
        model: String(row.fallback_model_name || ''),
      }
      : null,
...
async function getBibleGenerationJobById(
  db: D1Database,
  jobId: string,
  userId: string
): Promise<BibleGenerationJob | null> {
  ...
  if (!row) return null;
  return normalizeBibleGenerationJob(row);
}

async function createBibleGenerationJob(
  db: D1Database,
  userId: string,
  payload: BibleGenerationJobRequestPayload
): Promise<{ job: BibleGenerationJob; created: boolean }> {
  const jobId = crypto.randomUUID();
  await db.prepare(`
    INSERT INTO bible_generation_jobs (
      id,
      user_id,
      status,
      request_json,
    ...
    VALUES (?, ?, 'queued', ?, ?, (unixepoch() * 1000), (unixepoch() * 1000))
  `).bind(
    jobId,
    userId,
    JSON.stringify(payload),
    BIBLE_JOB_WAITING_MESSAGE
  ).run();

  const created = await getBibleGenerationJobById(db, jobId, userId);
  if (!created) {
    throw new Error('创建 Story Bible 任务失败');
  }

  return { job: created, created: true };
}

function buildBibleGenreTemplate(genre: string): string {
...
【注意事项】不能只靠战力，要有情感线、成长线（心境成长）、谜团揭示。`,
  ...
  return genre && genreTemplates[genre] ? genreTemplates[genre] : '';
}

async function generateBibleFromRequest(
  db: D1Database,
  aiConfig: AIConfig,
  request: BibleGenerationRequestInput
): Promise<BibleGenerationTaskResult> {
  const genreInput = typeof request.genreInput === 'string' ? request.genreInput.trim() : '';
  const themeInput = typeof request.themeInput === 'string' ? request.themeInput.trim() : '';
  const keywordsInput = typeof request.keywordsInput === 'string' ? request.keywordsInput.trim() : '';
  const templateId = typeof request.templateId === 'string' ? request.templateId.trim() : '';
  const templateSnapshotDate = typeof request.templateSnapshotDate === 'string' ? request.templateSnapshotDate.trim() : '';

  const bodyTemplate = request.bodyTemplate && typeof request.bodyTemplate === 'object'
    ? request.bodyTemplate as ImagineTemplate
    : null;
  ...
  const genreTemplate = buildBibleGenreTemplate(genre);
...
请基于以上信息，生成一个**能在番茄获得流量**的完整 Story Bible：`;
  ...
  try {
    ...
    if (isGeminiLikeConfig(aiConfig) && isLocationUnsupportedError(primaryError)) {
      const fallbackConfig = await getNonGeminiFallbackAIConfig(db, aiConfig);
      if (!fallbackConfig) {
        throw new Error(
...
type BibleQueuePayload = {
  taskType: 'bible';
  jobId: string;
};

async function enqueueBibleGenerationJob(
  c: any,
  payload: BibleQueuePayload
): Promise<void> {
  ...
  console.warn('GENERATION_QUEUE not bound, falling back to waitUntil for bible generation job');
  c.executionCtx.waitUntil(runBibleGenerationJob({ env: c.env, jobId: payload.jobId }));
}

export async function runBibleGenerationJob(params: { env: Env; jobId: string }) {
  const { env, jobId } = params;
  ...
  try {
    ...
    await env.DB.prepare(`
      UPDATE bible_generation_jobs
      SET status = 'running',
          message = ?,
          started_at = COALESCE(started_at, (unixepoch() * 1000)),
          updated_at = (unixepoch() * 1000)
      WHERE id = ?
    `).bind(BIBLE_JOB_RUNNING_MESSAGE, jobId).run();

    const runningRow = await env.DB.prepare(`
      SELECT user_id, request_json
      FROM bible_generation_jobs
      WHERE id = ?
      LIMIT 1
    `).bind(jobId).first() as { user_id: string; request_json: string } | null;

    if (!runningRow) {
      return;
    }

    const payload = parseJsonSafely<BibleGenerationJobRequestPayload>(
      runningRow.request_json,
      { request: {}, aiConfig: { provider: '', model: '', apiKey: '' } }
    );

    if (!payload.aiConfig || !payload.aiConfig.apiKey) {
      throw new Error('任务缺少可用的 AI 配置，无法继续执行');
    }

    const result = await generateBibleFromRequest(
      env.DB,
      payload.aiConfig,
      payload.request || {}
    );
    ...
    `).bind(
      result.bible,
      result.templateApplied ? JSON.stringify(result.templateApplied) : null,
      result.fallbackModelUsed?.provider || null,
      result.fallbackModelUsed?.model || null,
      'Story Bible 生成完成',
      jobId
    ).run();
  } catch (error) {
    const formatted = formatGenerationError(error);
    console.error(`Bible generation job ${jobId} failed:`, formatted);
    await env.DB.prepare(`
      UPDATE bible_generation_jobs
      SET status = 'failed',
          message = ?,
          ...
          updated_at = (unixepoch() * 1000)
      WHERE id = ?
    `).bind(formatted, formatted, jobId).run();
  }
}
...
generationRoutes.get('/bible-generation-jobs/:id', async (c) => {
  const userId = c.get('userId') as string | null;
  if (!userId) {
    return c.json({ success: false, error: 'Unauthorized' }, 401);
  }
  ...
  const jobId = c.req.param('id');
  const job = await getBibleGenerationJobById(c.env.DB, jobId, userId);
  if (!job) {
    return c.json({ success: false, error: 'Story Bible 任务不存在' }, 404);
  }
  ...
  return c.json({
    ...
    job: {
      ...
      bible: job.resultBible,
      templateApplied: job.templateApplied,
      fallbackModelUsed: job.fallbackModelUsed,
...
// Generate bible (queue-backed, returns immediately)
generationRoutes.post('/generate-bible', async (c) => {
  ...
  try {
    const body = await c.req.json().catch(() => ({} as any));
    const request: BibleGenerationRequestInput = {
      genreInput: typeof body.genre === 'string' ? body.genre.trim() : '',
      themeInput: typeof body.theme === 'string' ? body.theme.trim() : '',
      keywordsInput: typeof body.keywords === 'string' ? body.keywords.trim() : '',
      templateId: typeof body.templateId === 'string' ? body.templateId.trim() : '',
      templateSnapshotDate: typeof body.templateSnapshotDate === 'string' ? body.templateSnapshotDate.trim() : '',
      bodyTemplate: body.template && typeof body.template === 'object'
        ? body.template as ImagineTemplate
        : null,
    };

    const { job, created } = await createBibleGenerationJob(c.env.DB, userId, {
      request,
      aiConfig,
    });

    if (created) {
      await enqueueBibleGenerationJob(c, {
        taskType: 'bible',
        jobId: job.id,
      });
    }
